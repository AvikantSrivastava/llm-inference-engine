...
RUN mkdir -p /opt/llama \
    && curl -L -o /tmp/llama.tar.gz \
        https://github.com/ggml-org/llama.cpp/releases/download/b8934/llama-b8934-bin-ubuntu-vulkan-x64.tar.gz \
    && tar xf /tmp/llama.tar.gz -C /opt/llama \
    && mv /opt/llama/llama*/* /opt/llama/ \
    && rmdir /opt/llama/llama-b8934 \
    && rm -f /tmp/llama-b8934-bin-ubuntu-vulkan-*.tar.gz
